...
# Added on top of this skeleton code.
#
#
import sys
import torch
...
def train_epoch(model, loader, optimizer, device, epoch):
    ...
    for batch_idx, (images, labels) in enumerate(loader):
        ...
        if (batch_idx + 1) % 200 == 0:
            print(
                f"  Epoch {epoch} [{batch_idx+1}/{len(loader)}] Loss: {loss.item():.4f}"
            )
    return total_loss / total, correct / total


def evaluate(model, loader, device):
    """Evaluates the model on the given DataLoader and returns loss and accuracy."""
    model.eval()
    total_loss, correct, total = 0.0, 0, 0
    with torch.no_grad():
...
def main(argv):
    ...
    # select device — fall back to cuda or cpu if mps is unavailable
    ...
    for epoch in range(1, config.epochs + 1):
        tr_loss, tr_acc = train_epoch(model, train_loader, optimizer, device, epoch)
    ...
    print(config.config_string + f"{best_acc*100:.2f},{best_epoch}")
